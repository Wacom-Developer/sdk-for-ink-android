...
class InkBrushTool(context: Context) : RasterTool(context) {
    ...
    companion object {
        val uri = URIBuilder.getToolURI("raster", "ink_brush")
    }

    override var brush = BrushPalette.inkBrush(context)
    var previousSize = 0f
    ...
    override val touchCalculator: Calculator = { previous, current, next ->
        ...
        PathPoint(current.x, current.y, size = size, alpha = 1f)
    }

    override val stylusCalculator: Calculator = { previous, current, next ->
        // for felt pressure it is not take on account
        var size = current.computeValueBasedOnSpeed(
            previous,
            next,
            minValue = 5f,
        ...
        PathPoint(current.x, current.y, size = size, alpha = 0.5f)
    }
}